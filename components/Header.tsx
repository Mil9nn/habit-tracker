...
export default function Header({ title = "CaloMind" }: HeaderProps) {
  ...
  useEffect(() => {
    // If no profile exists, link to profile edit page
    if (!profile) {
  ...
  return (
    <>
      <header className="bg-[#F6F8FB] shadow-sm border-b fixed top-0 left-0 right-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            ...
            >
              ...
              <h1 className="text-base sm:text-xl font-bold text-gray-900">{formatTitle(title)}</h1>
            </div>
            ...
            >
              {session?.user?.image ? (
                ...
                <div className="h-9 w-9 rounded-full bg-gradient-to-br from-blue-400 to-red-500 flex items-center justify-center text-white text-sm font-semibold">
